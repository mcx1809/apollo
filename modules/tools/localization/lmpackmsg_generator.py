...
def write_info(source_tuple, filename):
	"""1. acquire the correct gps msg from odometrymsg according to the timestamp from obstaclemsg 
	   ...
	   4. set the property value of OdometryLaneMarkersPack according to ContourOdometryLaneMarkers
	   4. write the serialized string of OdometryLaneMarkersPack message to file
	Args: 
		tuple:(obstaclemsg_list, odometrymsg_list)
	...
	for obs_index in range(len(source_tuple[0])):                 # each message in obstacle message list
		for odo_index in range(len(source_tuple[1]) - 1):         # each message in odometry message list
		...
		localization_next   = source_tuple[1][odo_index + 1].localization.position
		
		left_lane_marker    = left_lane_marker_group.lane_marker.add()
		left_c0_position               = source_tuple[0][obs_index].lane_marker.left_lane_marker.c0_position
		...
		right_c0_position              = source_tuple[0][obs_index].lane_marker.right_lane_marker.c0_position
...
def parse_info(filename):
	"""Parse messages from protobuf message file 
	Args: 
		filename: protobuf message file name
	"""
	f = open(filename, "rb")
	parsed_odometry_lane_markers_pack = odometry_lane_marker_pb2.OdometryLaneMarkersPack()
	parsed_odometry_lane_markers_pack.ParseFromString(f.read())
	print(type(parsed_odometry_lane_markers_pack.lane_markers[0]))
	print(type(parsed_odometry_lane_markers_pack.lane_markers[0].lane_marker))
	print(type(parsed_odometry_lane_markers_pack.lane_markers[0].lane_marker[0]))
	print(type(parsed_odometry_lane_markers_pack.lane_markers[1]))
	print(type(parsed_odometry_lane_markers_pack.lane_markers[1].lane_marker))
	print(type(parsed_odometry_lane_markers_pack.lane_markers[1].lane_marker[0]))		
	f.close()
	
if __name__ == '__main__':
	resource = read_bag('/apollo/data/bag/2018-09-20-11-14-59/2018-09-20-11-15-00_0.bag')
	write_info(resource,"/apollo/modules/localization/testdata/OdometryLaneMarkers.pb")
	parse_info("/apollo/modules/localization/testdata/OdometryLaneMarkers.pb")
