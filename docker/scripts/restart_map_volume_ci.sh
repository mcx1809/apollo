...
if [[ ${MAP_VOLUME_CONF} == *"${MAP_VOLUME}"* ]]; then
  ...
  docker stop ${MAP_VOLUME}_ci > /dev/null 2>&1

  MAP_VOLUME_IMAGE=${DOCKER_REPO}:map_volume-${map_name}-${map_version}
  docker pull ${MAP_VOLUME_IMAGE}
  docker run -it -d --rm --name ${MAP_VOLUME}_ci ${MAP_VOLUME_IMAGE}
  MAP_VOLUME_CONF="${MAP_VOLUME_CONF} --volumes-from ${MAP_VOLUME}_ci"
fi
