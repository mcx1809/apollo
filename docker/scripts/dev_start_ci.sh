...
for map_name in ${DEFAULT_MAPS[@]}; do
    source ${APOLLO_ROOT_DIR}/docker/scripts/restart_map_volume_ci.sh ${map_name} "${VOLUME_VERSION}"
done
...
function local_volumes() {
    ...
    volumes="-v $APOLLO_ROOT_DIR:/apollo \
             -v /$USER/.cache:${DOCKER_HOME}/.cache"
    case "$(uname -s)" in
        Linux)
...
function main(){
    ...
    docker ps -a --format "{{.Names}}" | grep 'apollo_ci_dev' 1>/dev/null
    if [ $? == 0 ]; then
        docker stop apollo_ci_dev 1>/dev/null
        docker rm -f apollo_ci_dev 1>/dev/null
    fi
    local display=""
    if [[ -z ${DISPLAY} ]];then
        display=":0"
    ...
    docker stop ${LOCALIZATION_VOLUME}_ci > /dev/null 2>&1

    LOCALIZATION_VOLUME_IMAGE=${DOCKER_REPO}:localization_volume-${ARCH}-latest
    docker pull ${LOCALIZATION_VOLUME_IMAGE}
    docker run -it -d --rm --name ${LOCALIZATION_VOLUME}_ci ${LOCALIZATION_VOLUME_IMAGE}

    YOLO3D_VOLUME=apollo_yolo3d_volume
    docker stop ${YOLO3D_VOLUME}_ci > /dev/null 2>&1

    YOLO3D_VOLUME_IMAGE=${DOCKER_REPO}:yolo3d_volume-${ARCH}-latest
    docker pull ${YOLO3D_VOLUME_IMAGE}
    docker run -it -d --rm --name ${YOLO3D_VOLUME}_ci ${YOLO3D_VOLUME_IMAGE}

    info "Starting docker container \"apollo_ci_dev\" ..."
    docker run -it \
        -d \
        --privileged \
        --name apollo_ci_dev \
        ${MAP_VOLUME_CONF} \
        --volumes-from ${LOCALIZATION_VOLUME}_ci \
        --volumes-from ${YOLO3D_VOLUME}_ci \
        -e DISPLAY=$display \
        -e DOCKER_USER=$USER \
        -e USER=$USER \
    ...
    if [ $? -ne 0 ];then
        error "Failed to start docker container \"apollo_ci_dev\" based on image: $IMG"
        exit 1
    fi

    if [ "${USER}" != "root" ]; then
        docker exec apollo_ci_dev bash -c '/apollo/scripts/docker_adduser.sh'
    fi
